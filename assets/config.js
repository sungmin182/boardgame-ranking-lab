...
window.LZ_CONFIG = {
  ...
  LINKS: {
    ...
    daangn: {
      ...
      /*
       * 이름을 복사한 뒤 넘어갈 곳(안드로이드).
       *
       * 앱의 검색 화면으로는 갈 수 없으므로 플레이스토어의 당근 페이지로 보낸다.
       * 이미 깔려 있으면 그 페이지에 "열기" 버튼이 있어서 앱으로 넘어갈 수 있다.
       * market:// 은 브라우저가 아니라 플레이스토어 앱을 바로 연다.
       *
       * 이 줄을 지우면 복사만 하고 화면을 옮기지 않는다.
       */
      androidOpen: 'market://details?id=com.towneers.www',

      // 앱이 주소를 받아주게 되면 쓸 값(당근이 공개한 assetlinks.json 에서 확인).
      // 지금은 검색 주소를 안 받아 쓰이지 않는다.
